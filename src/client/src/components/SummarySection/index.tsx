...
const SummarySection = ({ selectionTitle, selectionRows, isEditable, removeAzureFunction, updateFunctionNames, functionApps }: Props) => {
    ...
    const renderTile = (title: string, svgUrl?: string, company?: string, originalTitle?: string, isEditable?: boolean, withIndent?: boolean, handleCloseClick?: (idx: number) => any, handleInputChange?: (newTitle: string, idx: number) => any, idx?: number) => {
        return (
          <div className={styles.tileContainer} key={idx}>
            <SummaryTile title={title} version="v1.0" svgUrl={svgUrl} company={company} originalTitle={originalTitle} isEditable={isEditable} withIndent={withIndent} handleCloseClick={handleCloseClick} handleInputChange={handleInputChange} idx={idx} />
          </div>
        )
    }
    return (
            ...
            <div className={styles.selectionTitle}>{selectionTitle}</div>
                {selectionRows.map((selection) => (
                    ...
                    {selection.functionNames && selection.functionNames.map((functionName, idx: number) => (
                        renderTile(functionName, undefined, undefined, undefined, true, true, handleRemoveFunction, handleAzureFuncNameChange, idx + 1)
                    ))}
                    </React.Fragment>
                ))}
        </div>
