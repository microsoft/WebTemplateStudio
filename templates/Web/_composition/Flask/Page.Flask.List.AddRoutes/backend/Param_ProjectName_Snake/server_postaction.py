...
def get_list():
    return flask.jsonify(Param_ProjectName_Snake.sample_data.sample_list)
...
def add_list_item():
    ...
    Param_ProjectName_Snake.sample_data.sample_list.insert(0, list_item)
...
def delete_list_item(item_id):
    item_to_remove = next(
        (item for item in Param_ProjectName_Snake.sample_data.sample_list if item["id"] == item_id),
        None,
    )
    ...
    Param_ProjectName_Snake.sample_data.sample_list.remove(item_to_remove)
