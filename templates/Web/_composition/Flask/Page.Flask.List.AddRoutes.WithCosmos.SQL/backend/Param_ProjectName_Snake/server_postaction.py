import flask
//{[{
import Param_ProjectName_Snake.sql.service
//}]}
...
def get_list():
    return flask.jsonify(Param_ProjectName_Snake.sql.service.get())


@app.route(ENDPOINT_LIST, methods=["POST"])
def add_list_item():
    json_response = flask.jsonify(Param_ProjectName_Snake.sql.service.create())
...
def delete_list_item(item_id):
    try:
        removed_item = flask.jsonify(Param_ProjectName_Snake.sql.service.delete(item_id))
        return removed_item
